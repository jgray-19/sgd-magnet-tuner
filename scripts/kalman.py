...
X0 = {{ 
    x  = 1e-2 / sqrt(tws["$start"].beta11),
    px = -1e-5,
    y  = 1e-2 / sqrt(tws["$start"].beta22),
    py = -1e-5,
    t  = 0,
    pt = 0
}}
...
"""
    ...
    if ACD_ON:
        meas_df = meas_df[
            (meas_df.index > RAMP_UP_TURNS) & (meas_df.index <= n_turns + RAMP_UP_TURNS)
        ]

    true_data_df = tfs.read(TRACK_DATA_FILE, index="turn")
    if ACD_ON:
        true_data_df = true_data_df[
            (true_data_df.index > RAMP_UP_TURNS)
            & (true_data_df.index <= n_turns + RAMP_UP_TURNS)
        ]
    ...
    # 3) Covariances
    Q = (1e-3) ** 2 * np.diag(
        [sigma_x**2, (sigma_x / 10) ** 2, sigma_y**2, (sigma_y / 10) ** 2]
    )
    #      x         p_x         y         p_y
    ...
    for plane, i in zip(["x", "px", "y", "py"], range(4)):
        print(
            f"RMS filtered {plane} error:",
            np.sqrt(np.mean((x_hat[..., i] - true_data[..., i]) ** 2)),
        )
        print(
            f"RMS raw      {plane} error:",
    ...
    plt.subplot(1, 2, 1)
    plt.scatter(
        x_hat[:, bpm_index, 0], x_hat[:, bpm_index, 1], s=1, c="blue", label="Kalman"
    )
    plt.scatter(
        true_data[:, bpm_index, 0],
    ...
    plt.scatter(
        meas[:, bpm_index, 0], meas[:, bpm_index, 1], s=1, c="green", label="Measured"
    )
    ...
    plt.subplot(1, 2, 2)
    plt.scatter(
        x_hat[:, bpm_index, 2], x_hat[:, bpm_index, 3], s=1, c="blue", label="Kalman"
    )
    plt.scatter(
        true_data[:, bpm_index, 2],
    ...
    plt.scatter(
        meas[:, bpm_index, 2], meas[:, bpm_index, 3], s=1, c="green", label="Measured"
    )

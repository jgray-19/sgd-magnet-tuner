...
def make_noisy_track_data(orig_data: tfs.TfsDataFrame, seed=42, info=True) -> tuple[tfs.TfsDataFrame, tfs.TfsDataFrame]:
    """
    ...
    data = orig_data.copy(deep=False)  # share underlying data, just new view
    data["x"] = orig_data["x"] + np.random.normal(0, POSITION_STD_DEV, size=len(orig_data))
    data["y"] = orig_data["y"] + np.random.normal(0, POSITION_STD_DEV, size=len(orig_data))
    ...
    # prev/next BPMs
    prev_x_df = prev_bpm_to_pi_2(tws["mu1"], tws.q1).rename(columns={"prev_bpm": "prev_bpm_x", "delta": "delta_x"})
    prev_y_df = prev_bpm_to_pi_2(tws["mu2"], tws.q2).rename(columns={"prev_bpm": "prev_bpm_y", "delta": "delta_y"})
    next_x_df = next_bpm_to_pi_2(tws["mu1"], tws.q1).rename(columns={"next_bpm": "next_bpm_x", "delta": "delta_x"})
    next_y_df = next_bpm_to_pi_2(tws["mu2"], tws.q2).rename(columns={"next_bpm": "next_bpm_y", "delta": "delta_y"})
    ...
    for col, mapper in (("sqrt_betax", map_sqrt_betax), ("sqrt_betay", map_sqrt_betay), ("alfax", map_alfax), ("alfay", map_alfay)):
    ...
    coords_x_p = coords_p.rename(columns={"turn": "turn_x_p", "name": "prev_bpm_x", "x": "prev_x"}).loc[:, ["turn_x_p", "prev_bpm_x", "prev_x"]]
    coords_y_p = coords_p.rename(columns={"turn": "turn_y_p", "name": "prev_bpm_y", "y": "prev_y"}).loc[:, ["turn_y_p", "prev_bpm_y", "prev_y"]]
    coords_x_n = coords_n.rename(columns={"turn": "turn_x_n", "name": "next_bpm_x", "x": "next_x"}).loc[:, ["turn_x_n", "next_bpm_x", "next_x"]]
    coords_y_n = coords_n.rename(columns={"turn": "turn_y_n", "name": "next_bpm_y", "y": "next_y"}).loc[:, ["turn_y_n", "next_bpm_y", "next_y"]]
    ...
    data_p = data_p.merge(coords_x_p, on=["turn_x_p", "prev_bpm_x"], how="left", copy=False)
    data_p = data_p.merge(coords_y_p, on=["turn_y_p", "prev_bpm_y"], how="left", copy=False)
    data_n = data_n.merge(coords_x_n, on=["turn_x_n", "next_bpm_x"], how="left", copy=False)
    data_n = data_n.merge(coords_y_n, on=["turn_y_n", "next_bpm_y"], how="left", copy=False)
    ...
    data_n.iloc[-1, data_n.columns.get_loc("px")] = data_p.iloc[-1, data_p.columns.get_loc("px")]
    data_n.iloc[-1, data_n.columns.get_loc("py")] = data_p.iloc[-1, data_p.columns.get_loc("py")]
    data_p.iloc[0, data_p.columns.get_loc("px")] = data_n.iloc[0, data_n.columns.get_loc("px")]
    data_p.iloc[0, data_p.columns.get_loc("py")] = data_n.iloc[0, data_n.columns.get_loc("py")]

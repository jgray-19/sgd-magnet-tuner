...
def prepare_plot_context(
    *,
    ...
    if estimate_source == "none":
        ...
        if estimate_source == "estimates":
            estimates = convert_estimates_to_optimisation_space(
                accelerator,
                estimates,
                tune_knobs_file,
                corrector_file,
            )
            uncertainties = convert_uncertainties_to_optimisation_space(
                accelerator,
                uncertainties,
                tune_knobs_file,
                corrector_file,
            ...
            LOGGER.info(
                "Using checkpoint estimates directly in optimisation-space knob coordinates"
            )
...
def load_estimates_from_checkpoints(
    ...
    Uses ``best_knobs`` when available, falling back to ``current_knobs``.
    """
...
def plot_quad_diffs(
    ...
    In full-ring mode, all knobs are expected under the ``Arc 1`` header.
    Strengths and quadrupole displacements are plotted separately.
    """
    ...
    if fullring:
        ...
        # Separate strength knobs into quads, bends, and others.
        ...
        if accelerator is not None:
            ...
            if quads_without_pos:
                # If a quadrupole cannot be placed along s, move it to the others panel.
        ...
        plt.tight_layout()
        fig_main.savefig(results_dir / f"quad_diffs_{squeeze_step}_fullring.png")
        ...
        plt.tight_layout()
        fig_bends.savefig(results_dir / f"quad_diffs_{squeeze_step}_fullring_bends.png")
        return
...
def plot_phase_advances(
    ...
    **kwargs,
    ...
    from aba_optimiser.measurements.plot_quad_diffs_arc import plot_phase_advances as impl
...
def plot_fullring_comparison(
    ...
    **kwargs,
    ...
    from aba_optimiser.measurements.plot_quad_diffs_fullring import (
        plot_fullring_comparison as impl,
    )
...
def main():
    ...
    if args.fullring:
        from aba_optimiser.measurements.plot_quad_diffs_fullring import main as impl
    else:
        from aba_optimiser.measurements.plot_quad_diffs_arc import main as impl
    impl()

"""Run interaction-region optimisations using all configured BPM ranges."""
...
@dataclass
class RangeConfig:
    """Grouped BPM range definitions for one set of optimisation segments."""
...
class IterationConfig:
    """One full optimisation iteration with input files and labels."""
...
def process_single_config(
    ...
    """Process a single measurement configuration using all measurement points.
    ...
    # Use all measurement points directly - no averaging
    logger.info(f"Using all {len(pzs)} measurement points for optimisation")
    logger.info(f"Number of unique BPMs: {pzs['name'].nunique()}")
    logger.info(f"Number of turns: {pzs['turn'].nunique()}")
    ...
    # Save all points to parquet file
    pzs.to_parquet(file_path)

    # Calculate number of tracks and turns from the data
    num_tracks = len(files)  # Number of measurement files
    # Get actual number of turns per BPM (should be consistent)
    ...
    # Distribute tracks across workers for parallel processing
    # total_tracks = tracks_per_worker * num_batches * num_workers should equal num_tracks
    num_workers = min(num_tracks, 5)  # Cap at 1 workers
    tracks_per_worker = num_tracks // num_workers
    num_batches = 20

    simulation_config = SimulationConfig(
        # Use all measurement points with parallelization
    ...
    logger.info(f"optimisation config: {num_tracks} tracks, {turns_per_bpm} turns per track")

...
def test_matcher_beta_correction(tmp_dir: Path, seq_b1: Path, json_b1: Path) -> None:
    ...
    # # Read estimated strengths from file (assumed to be generated by controller test)
    # data_dir = Path(__file__).parent.parent.parent / "data"
    # estimated_strengths_file = data_dir / "estimated_quad_strengths.json"

    # if not estimated_strengths_file.exists():
    #     pytest.skip(f"Estimated strengths file not found: {estimated_strengths_file}. Run controller test first.")

    # with open(estimated_strengths_file, "r") as f:
    #     all_estimates = json.load(f)

    # print(f"Loaded estimated strengths from {estimated_strengths_file}")
    ...
    # Use the actual perturbed magnet strengths for testing
    all_estimates = {k: v for k, v in magnet_strengths.items() if k.startswith("MQ")}
    print(f"Using {len(all_estimates)} estimated quadrupole strengths for beta matching test")
    print("Using actual perturbed magnet strengths for beta matching test")
    ...
    knobs_file = Path("/afs/cern.ch/work/j/jmgray/private/omc3/omc3/model/accelerators/lhc/2025/correctors/correctors_b1/beta_correctors.json")
    ...
    # interface.set_magnet_strengths(magnet_strengths)
